...
public class CNCtx implements javax.naming.Context {
    ...
    private static boolean isCorbaUrl(String url) {
        return url.startsWith("iiop://") //Audit Issue: User marked 'iiop://' string as Not an Issue string with comment: ''.
            || url.startsWith("iiopname://") //Audit Issue: User marked 'iiopname://' string as Not an Issue string with comment: ''.
    ...
    private String initUsingUrl(ORB orb, String url, Hashtable env)
        throws NamingException {
        if (url.startsWith("iiop://") || url.startsWith("iiopname://")) { //Audit Issue: User marked 'iiopname://' string as Not an Issue string with comment: ''.
    ...
    private String initUsingIiopUrl(ORB defOrb, String url, Hashtable env)
        ...
        try {
            ...
            for (int i = 0; i < addrs.size(); i++) {
                ...
                try {
                    try {
                        String tmpUrl = "corbaloc:iiop:" + addr.host //Audit Issue: User marked 'corbaloc:iiop:' string as Not an Issue string with comment: ''.
...
"COS Name Service not registered with ORB under the name 'NameService'");
            ...
            NamingException ne = new ConfigurationException(
                "Invalid URL or IOR: " + ncIor); //Audit Issue: User marked 'Invalid URL or IOR: ' string as Not an Issue string with comment: ''.
            ne.setRootCause(e);
            throw ne;
        } catch (org.omg.CORBA.INV_OBJREF e) {
            NamingException ne = new ConfigurationException(
                "Invalid object reference: " + ncIor); //Audit Issue: User marked 'Invalid object reference: ' string as Not an Issue string with comment: ''.
            ne.setRootCause(e);
            throw ne;
        }
    }
    ...
    private String getStringifiedIor(String url) throws NamingException {
        if (url.startsWith("IOR:") || url.startsWith("corbaloc:")) {
            ...
            try {
                ...
                NamingException ne =
                    new ConfigurationException("Invalid URL: " + url); //Audit Issue: User marked 'Invalid URL: ' string as Not an Issue string with comment: ''.
                ne.setRootCause(e);
                throw ne;
            } finally {
                try {
