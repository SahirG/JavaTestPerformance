    ...
    private static byte[] getEncodedKey(Key key) throws InvalidKeyException {
        ...
        if ((encodedKey.length < 5) || (encodedKey.length > 128)) {
            throw new InvalidKeyException
                ("Key length must be between 40 and 1024 bit"); //Audit Issue: User marked 'Key length must be between 40 and 1024 bit' string as Not an Issue string with comment: ''.
        }
        return encodedKey;
    }
