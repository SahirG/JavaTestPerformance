        ...
        if (streamFormatVersion == 2) {
            if (!(orbStream instanceof ValueInputStream)) {
                SystemException sysex = omgWrapper.notAValueinputstream(
                    CompletionStatus.COMPLETED_MAYBE);
                // XXX I18N?  Logging for IOException?
                IOException result = new IOException("Not a ValueInputStream"); //Audit Issue: User marked 'Not a ValueInputStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Not a ValueInputStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Not a ValueInputStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Not a ValueInputStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Not a ValueInputStream' string as Not an Issue string with comment: ''.
                result.initCause( sysex ) ;
                throw result;
            }
        }
...
// d11623 : implement readFully, required for serializing some core classes
    ...
    public final synchronized void registerValidation(ObjectInputValidation obj,
        ...
        // XXX I18N, logging needed.
        throw new Error("Method registerValidation not supported"); //Audit Issue: User marked 'Method registerValidation not supported' string as Not an Issue string with comment: ''.
    }
    ...
    static class ActiveRecursionManager
    {
        ...
        public Object getObject(int offset) throws IOException {
            ...
            if (!offsetToObjectMap.containsKey(position))
                // XXX I18N, logging needed.
                throw new IOException("Invalid indirection to offset " //Audit Issue: User marked 'Invalid indirection to offset ' string as Not an Issue string with comment: ''.
                                      + offset);
