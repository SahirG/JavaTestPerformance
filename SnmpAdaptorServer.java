...
public class SnmpAdaptorServer extends CommunicatorServer
    ...
    public void snmpV2Trap(SnmpOid trapOid, SnmpVarBindList varBindList)
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag,
                "snmpV2Trap", "trapOid=" + trapOid); //Audit Issue: User marked 'trapOid=' string as Not an Issue string with comment: ''.
        }
    ...
    private void snmpV2Trap(InetAddress addr,
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            final StringBuilder strb = new StringBuilder()
                .append("trapOid=").append(trapOid) //Audit Issue: User marked 'trapOid=' string as Not an Issue string with comment: ''.
                .append("\ncommunity=").append(cs)
                .append("\naddr=").append(addr)
    ...
    public Vector snmpInformRequest(SnmpInformHandler cb, SnmpOid trapOid,
        ...
        if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINER, dbgTag,
                "snmpInformRequest", "trapOid=" + trapOid); //Audit Issue: User marked 'trapOid=' string as Not an Issue string with comment: ''.
        }
