    ...
    implements BufferManagerRead, MarkAndResetHandler
{
    ...
    public void processFragment(ByteBuffer byteBuffer, FragmentMessage msg)
    {
        ...
        synchronized (fragmentQueue) {
            if (debug)
            {
                ...
                sb.append("processFragment() - queueing ByteBuffer id ("); //Audit Issue: User marked 'processFragment() - queueing ByteBuffer id (' string as Not an Issue string with comment: ''.
      ...
      try {
        ...
        synchronized (fragmentQueue) {
            ...
            if (debug)
            {
                ...
                sb1.append("underflow() - dequeued ByteBuffer id ("); //Audit Issue: User marked 'underflow() - dequeued ByteBuffer id (' string as Not an Issue string with comment: ''.
            ...
            if (markEngaged == false && bbwi != null && bbwi.byteBuffer != null)
            {
                ...
                if (debug)
                {
                    ...
                    StringBuffer sb = new StringBuffer(80);
                    sb.append("underflow() - releasing ByteBuffer id ("); //Audit Issue: User marked 'underflow() - releasing ByteBuffer id (' string as Not an Issue string with comment: ''.
                        ...
                        if (inputBbAddress != bbAddress)
                        {
                            if (debug)
                            {
                                 ...
                                 sb.append("close() - fragmentQueue is ")
                                   .append("releasing ByteBuffer id (") //Audit Issue: User marked 'releasing ByteBuffer id (' string as Not an Issue string with comment: ''.
                   ...
                   if (inputBbAddress != bbAddress)
                   {
                       if (debug)
                       {
                            ...
                            sb.append("close() - fragmentStack - releasing ")
                              .append("ByteBuffer id (" + bbAddress + ") to ") //Audit Issue: User marked 'ByteBuffer id (' string as Not an Issue string with comment: ''.
                              .append("ByteBufferPool.");
                            String msg = sb.toString();
                            dprint(msg);
                       }
    ...
    {
        ORBUtility.dprint("BufferManagerReadStream", msg); //Audit Issue: User marked 'BufferManagerReadStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'BufferManagerReadStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'BufferManagerReadStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'BufferManagerReadStream' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'BufferManagerReadStream' string as Not an Issue string with comment: ''.
    }
