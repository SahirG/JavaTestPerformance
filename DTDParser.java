...
public class DTDParser {
    ...
    public final static String TYPE_ID = "ID"; //Audit Issue: User marked 'ID' string as Not an Issue string with comment: ''.
    public final static String TYPE_IDREF = "IDREF"; //Audit Issue: User marked 'IDREF' string as Not an Issue string with comment: ''.
    public final static String TYPE_IDREFS = "IDREFS"; //Audit Issue: User marked 'IDREFS' string as Not an Issue string with comment: ''.
...
//    private SimpleHashtable    elements = new SimpleHashtable (47);
    ...
    public void setLocale(Locale l) throws SAXException {
        ...
        if (l != null && !messages.isLocaleSupported(l.toString())) {
            throw new SAXException(messages.getMessage(locale,
                    "P-078", new Object[]{l})); //Audit Issue: User marked 'P-078' string as Not an Issue string with comment: ''.
        }
        locale = l;
    }
    ...
    public void parse(String uri)
        ...
        if (inSource == null) {
            ...
            warning("P-065", null); //Audit Issue: User marked 'P-065' string as Not an Issue string with comment: ''.
...
//        isInAttribute = false;
        ...
        builtin("amp", "&#38;");
        builtin("lt", "&#60;"); //Audit Issue: User marked '&#60;' string as Not an Issue string with comment: ''.
        builtin("gt", ">");
        builtin("quot", "\"");
        builtin("apos", "'");
    ...
    private void parseInternal(InputSource input)
        ...
        if (input == null)
            fatal("P-000"); //Audit Issue: User marked 'P-000' string as Not an Issue string with comment: ''.
        ...
        try {
            ...
            if (!in.isEOF()) {
                fatal("P-001", new Object[] //Audit Issue: User marked 'P-001' string as Not an Issue string with comment: ''.
            ...
            if (!in.isDocument()) {
                ...
                fatal("P-002", new Object[]{name}); //Audit Issue: User marked 'P-002' string as Not an Issue string with comment: ''.
            } else {
                fatal("P-003", null); //Audit Issue: User marked 'P-003' string as Not an Issue string with comment: ''.
            }
...
//        elements.clear();
    ...
    void afterRoot() throws SAXException {
        ...
        for (Enumeration e = ids.keys();
            ...
            if (Boolean.FALSE == value)
                error("V-024", new Object[]{id}); //Audit Issue: User marked 'V-024' string as Not an Issue string with comment: ''.
        }
    }
    ...
    private void whitespace(String roleId)
        ...
        // [3] S ::= (#x20 | #x9 | #xd | #xa)+
        if (!maybeWhitespace()) {
            fatal("P-004", new Object[] //Audit Issue: User marked 'P-004' string as Not an Issue string with comment: ''.
    ...
    private String getNmtoken()
        ...
        if (!XmlChars.isNameChar(c))
            fatal("P-006", new Object[]{new Character(c)}); //Audit Issue: User marked 'P-006' string as Not an Issue string with comment: ''.
    ...
    private void parseLiteral(boolean isEntityValue)
        ...
        if (quote != '\'' && quote != '"') {
            fatal("P-007"); //Audit Issue: User marked 'P-007' string as Not an Issue string with comment: ''.
        }
        ...
        for (; ;) {
            ...
            if (c == '&') {
                ...
                if (entityName != null) {
                    ...
                    fatal("P-009"); //Audit Issue: User marked 'P-009' string as Not an Issue string with comment: ''.
                continue;
            ...
            if (c == '%' && isEntityValue) {
                ...
                if (entityName != null) {
                    ...
                    fatal("P-011"); //Audit Issue: User marked 'P-011' string as Not an Issue string with comment: ''.
            }
            ...
            if (!isEntityValue) {
                ...
                if (c == '<')
                    fatal("P-012"); //Audit Issue: User marked 'P-012' string as Not an Issue string with comment: ''.
            }
...
//    isInAttribute = false;
    ...
    private void expandEntityInLiteral(String name, SimpleHashtable table,
        ...
        if (entity instanceof InternalEntity) {
            ...
            if (!isEntityValue)    // must be a PE ...
                fatal("P-013", new Object[]{name}); //Audit Issue: User marked 'P-013' string as Not an Issue string with comment: ''.
            ...
            //
            fatal((table == params) ? "V-022" : "P-014", //Audit Issue: User marked 'P-014' string as Not an Issue string with comment: ''.
                    new Object[]{name});
        }
    }
    ...
    private String getQuotedString(String type, String extra)
        ...
        if (quote != '\'' && quote != '"')
            fatal("P-015", new Object[]{ //Audit Issue: User marked 'P-015' string as Not an Issue string with comment: ''.
    ...
    private String parsePublicId() throws IOException, SAXException {
        ...
        String retval = getQuotedString("F-033", null); //Audit Issue: User marked 'F-033' string as Not an Issue string with comment: ''.
        for (int i = 0; i < retval.length(); i++) {
            char c = retval.charAt(i);
            if (" \r\n-'()+,./:=?;!*#@$_%0123456789".indexOf(c) == -1
                    && !(c >= 'A' && c <= 'Z')
                    && !(c >= 'a' && c <= 'z'))
                fatal("P-016", new Object[]{new Character(c)}); //Audit Issue: User marked 'P-016' string as Not an Issue string with comment: ''.
        }
        strTmp = new StringBuffer();
    ...
    private boolean maybeComment(boolean skipStart)
        ...
        for (; ;) {
            try {
                ...
                //
                if (in.isInternal()) {
                    error("V-021", null); //Audit Issue: User marked 'V-021' string as Not an Issue string with comment: ''.
                }
                fatal("P-017"); //Audit Issue: User marked 'P-017' string as Not an Issue string with comment: ''.
            }
        }
    ...
    private boolean maybePI(boolean skipStart)
        ...
        if (target == null) {
            fatal("P-018"); //Audit Issue: User marked 'P-018' string as Not an Issue string with comment: ''.
        }
        if ("xml".equals(target)) {
            fatal("P-019"); //Audit Issue: User marked 'P-019' string as Not an Issue string with comment: ''.
        }
        if ("xml".equalsIgnoreCase(target)) {
            fatal("P-020", new Object[]{target}); //Audit Issue: User marked 'P-020' string as Not an Issue string with comment: ''.
        }
        ...
        if (maybeWhitespace()) {
            ...
            try {
                ...
                fatal("P-021"); //Audit Issue: User marked 'P-021' string as Not an Issue string with comment: ''.
            }
            dtdHandler.processingInstruction(target, strTmp.toString());
        } else {
            if (!in.peek("?>", null)) {
                fatal("P-022"); //Audit Issue: User marked 'P-022' string as Not an Issue string with comment: ''.
            }
    ...
    private String maybeReadAttribute(String name, boolean must)
        ...
        if (!maybeWhitespace()) {
            ...
            fatal("P-024", new Object[]{name}); //Audit Issue: User marked 'P-024' string as Not an Issue string with comment: ''.
            // NOTREACHED
        }
        ...
        return getQuotedString("F-035", name); //Audit Issue: User marked 'F-035' string as Not an Issue string with comment: ''.
    }
    ...
    private void readVersion(boolean must, String versionNum)
        ...
        if (must && value == null)
            fatal("P-025", new Object[]{versionNum}); //Audit Issue: User marked 'P-025' string as Not an Issue string with comment: ''.
        if (value != null) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (!((c >= '0' && c <= '9')
                    ...
                    fatal("P-026", new Object[]{value}); //Audit Issue: User marked 'P-026' string as Not an Issue string with comment: ''.
            }
        }
        if (value != null && !value.equals(versionNum))
            error("P-027", new Object[]{versionNum, value}); //Audit Issue: User marked 'P-027' string as Not an Issue string with comment: ''.
    }
    ...
    private String getMarkupDeclname(String roleId, boolean qname)
        ...
        name = maybeGetName();
        if (name == null)
            fatal("P-005", new Object[] //Audit Issue: User marked 'P-005' string as Not an Issue string with comment: ''.
            {messages.getMessage(locale, roleId)});
        return name;
    }
...
*/
    private boolean maybeElementDecl()
        ...
        String name = getMarkupDeclname("F-015", true); //Audit Issue: User marked 'F-015' string as Not an Issue string with comment: ''.
...
*/
        if (declaredElements.contains(name))
            error("V-012", new Object[]{name}); //Audit Issue: User marked 'V-012' string as Not an Issue string with comment: ''.
        else {
...
//        declEffective = true;
        ...
        short modelType;
        whitespace("F-000"); //Audit Issue: User marked 'F-000' string as Not an Issue string with comment: ''.
        if (peek(strEMPTY)) {
...
///        element.setContentModel(new StringModel(StringModelType.ANY));
        ...
        char c = getc();
        if (c != '>')
            fatal("P-036", new Object[]{name, new Character(c)}); //Audit Issue: User marked 'P-036' string as Not an Issue string with comment: ''.
        if (start != in)
            error("V-013", null); //Audit Issue: User marked 'V-013' string as Not an Issue string with comment: ''.
...
///<-
                ...
                fatal((type == 0) ? "P-039" : //Audit Issue: User marked 'P-039' string as Not an Issue string with comment: ''.
                        ((type == ',') ? "P-037" : "P-038"),
...
//                }
                if (c == type) {
                    ...
                    fatal((type == 0) ? "P-041" : "P-040", //Audit Issue: User marked 'P-040' string as Not an Issue string with comment: ''.
                            new Object[]{
                                new Character(c),
                                new Character(type)
                            });
...
//                current.addChild(temp);
        ...
        if (in != start)
            error("V-014", new Object[]{elementName}); //Audit Issue: User marked 'V-014' string as Not an Issue string with comment: ''.
        strTmp.append(')');
...
//        return retval;
    ...
    private void getMixed(String elementName, /*Element element,*/ InputEntity start)
        ...
        //        | '(' S? '#PCDATA'                   S? ')'
        maybeWhitespace();
        if (peek("\u0029*") || peek("\u0029")) { //Audit Issue: User marked '\u0029' string as Not an Issue string with comment: ''.
            if (in != start)
                error("V-014", new Object[]{elementName}); //Audit Issue: User marked 'V-014' string as Not an Issue string with comment: ''.
            strTmp.append(')');
...
//    l.add(new StringModel(StringModelType.PCDATA));
        ...
        while (peek("|")) {
            ...
            name = maybeGetName();
            if (name == null)
                fatal("P-042", new Object[] //Audit Issue: User marked 'P-042' string as Not an Issue string with comment: ''.
                {elementName, Integer.toHexString(getc())});
            if (l.contains(name)) {
                error("V-015", new Object[]{name}); //Audit Issue: User marked 'V-015' string as Not an Issue string with comment: ''.
            } else {
                l.add(name);
        ...
        if (!peek("\u0029*"))    // right paren //Audit Issue: User marked '\u0029*' string as Not an Issue string with comment: ''.
            fatal("P-043", new Object[] //Audit Issue: User marked 'P-043' string as Not an Issue string with comment: ''.
...
//       element.setContentModel(cm);
    ...
    private boolean maybeAttlistDecl()
        ...
        String elementName = getMarkupDeclname("F-016", true); //Audit Issue: User marked 'F-016' string as Not an Issue string with comment: ''.
//    Element    element = (Element) elements.get (name);
...
//    }
        ...
        while (!peek(">")) {
            ...
            if (c == '%') {
                ...
                if (entityName != null) {
                    nextChar(';', "F-021", entityName);
                    whitespace("F-021"); //Audit Issue: User marked 'F-021' string as Not an Issue string with comment: ''.
                    continue;
                } else
                    fatal("P-011"); //Audit Issue: User marked 'P-011' string as Not an Issue string with comment: ''.
            }
            ...
            if (attName == null) {
                fatal("P-044", new Object[]{new Character(getc())}); //Audit Issue: User marked 'P-044' string as Not an Issue string with comment: ''.
            }
            whitespace("F-001"); //Audit Issue: User marked 'F-001' string as Not an Issue string with comment: ''.
...
///            element.setId(name);
            ...
            //        (S? '|' S? Name)* S? ')'
                ...
                whitespace("F-002"); //Audit Issue: User marked 'F-002' string as Not an Issue string with comment: ''.
                nextChar('(', "F-029", null);
                maybeWhitespace();
                ...
                do {
                    ...
                    if ((name = maybeGetName()) == null)
                        fatal("P-068"); //Audit Issue: User marked 'P-068' string as Not an Issue string with comment: ''.
...
///                a.setValue(i, (String)v.elementAt(i));
                ...
                fatal("P-045", //Audit Issue: User marked 'P-045' string as Not an Issue string with comment: ''.
            ...
            //        | (('#FIXED' S)? AttValue)
            whitespace("F-003"); //Audit Issue: User marked 'F-003' string as Not an Issue string with comment: ''.
            if (peek("#REQUIRED"))
...
///            if (a.type() == Attribute.ID)
                if (typeName == TYPE_ID)
                    error("V-017", new Object[]{attName}); //Audit Issue: User marked 'V-017' string as Not an Issue string with comment: ''.
///            a.setIsFixed(true);
                attributeUse = DTDEventListener.USE_FIXED;
                whitespace("F-004"); //Audit Issue: User marked 'F-004' string as Not an Issue string with comment: ''.
                parseLiteral(false);
...
///            if (a.type() == Attribute.ID)
                if (typeName == TYPE_ID)
                    error("V-018", new Object[]{attName}); //Audit Issue: User marked 'V-018' string as Not an Issue string with comment: ''.
                parseLiteral(false);
///            if (a.type() != Attribute.CDATA)
///                a.setDefaultValue(normalize(false));
///            else
...
///        }
    ...
    private boolean maybeConditionalSect()
        ...
        if ((keyword = maybeGetName()) == null)
            fatal("P-046"); //Audit Issue: User marked 'P-046' string as Not an Issue string with comment: ''.
        maybeWhitespace();
        nextChar('[', "F-030", null);
        ...
        if ("INCLUDE".equals(keyword)) {
            for (; ;) {
                while (in.isEOF() && in != start)
                    in = in.pop();
                if (in.isEOF()) {
                    error("V-020", null); //Audit Issue: User marked 'V-020' string as Not an Issue string with comment: ''.
                }
                if (peek("]]>"))
                    break;
                ...
                fatal("P-047"); //Audit Issue: User marked 'P-047' string as Not an Issue string with comment: ''.
            }
            ...
            fatal("P-048", new Object[]{keyword}); //Audit Issue: User marked 'P-048' string as Not an Issue string with comment: ''.
        return true;
    }
    ...
    private int parseCharNumber()
        ...
        if (getc() != 'x') {
            ...
            for (; ;) {
                ...
                if (c == ';')
                    return retval;
                fatal("P-049"); //Audit Issue: User marked 'P-049' string as Not an Issue string with comment: ''.
            }
        } else
            for (; ;) {
                c = getc();
                ...
                if (c == ';')
                    return retval;
                fatal("P-050"); //Audit Issue: User marked 'P-050' string as Not an Issue string with comment: ''.
            }
    }
    ...
    private int surrogatesToCharTmp(int ucs4)
        ...
        fatal("P-051", new Object[]{Integer.toHexString(ucs4)}); //Audit Issue: User marked 'P-051' string as Not an Issue string with comment: ''.
        // NOTREACHED
        return -1;
    }
    ...
    private boolean maybePEReference()
        ...
        if (entity instanceof InternalEntity) {
            ...
            error("V-022", new Object[]{name}); //Audit Issue: User marked 'V-022' string as Not an Issue string with comment: ''.
        }
        return true;
    }
    ...
    private boolean maybeEntityDecl()
        ...
        doLexicalPE = false;
        whitespace("F-005"); //Audit Issue: User marked 'F-005' string as Not an Issue string with comment: ''.
        if (in.peekc('%')) {
            whitespace("F-006"); //Audit Issue: User marked 'F-006' string as Not an Issue string with comment: ''.
            defns = params;
        } else
            defns = entities;
        ...
        doLexicalPE = true;
        entityName = getMarkupDeclname("F-017", false); //Audit Issue: User marked 'F-017' string as Not an Issue string with comment: ''.
        whitespace("F-007"); //Audit Issue: User marked 'F-007' string as Not an Issue string with comment: ''.
        ...
        if (!doStore && defns == entities)
            warning("P-054", new Object[]{entityName}); //Audit Issue: User marked 'P-054' string as Not an Issue string with comment: ''.
        ...
        if (externalId == null) {
            ...
            if (defns == entities && maybeWhitespace()
                    && peek("NDATA")) {
                externalId.notation = getMarkupDeclname("F-018", false); //Audit Issue: User marked 'F-018' string as Not an Issue string with comment: ''.
    ...
    private ExternalEntity maybeExternalID()
        ...
        if (peek("PUBLIC")) {
            whitespace("F-009"); //Audit Issue: User marked 'F-009' string as Not an Issue string with comment: ''.
            temp = parsePublicId();
        } else if (!peek("SYSTEM"))
            return null;
        ...
        whitespace("F-008"); //Audit Issue: User marked 'F-008' string as Not an Issue string with comment: ''.
    ...
    private String parseSystemId()
        ...
        String uri = getQuotedString("F-034", null); //Audit Issue: User marked 'F-034' string as Not an Issue string with comment: ''.
        ...
        if (temp == -1 || uri.indexOf('/') < temp) {
            ...
            if (baseURI == null)
                fatal("P-055", new Object[]{uri}); //Audit Issue: User marked 'P-055' string as Not an Issue string with comment: ''.
            if (uri.length() == 0)
                uri = ".";
        ...
        if (uri.indexOf('#') != -1)
            error("P-056", new Object[]{uri}); //Audit Issue: User marked 'P-056' string as Not an Issue string with comment: ''.
        return uri;
    }
    ...
    private void maybeTextDecl()
        ...
        if (peek("<?xml")) {
            ...
            maybeWhitespace();
            if (!peek("?>"))
                fatal("P-057"); //Audit Issue: User marked 'P-057' string as Not an Issue string with comment: ''.
        }
    }
    ...
    private void externalParameterEntity(ExternalEntity next)
        ...
        if (!pe.isEOF())
            fatal("P-059", new Object[]{in.getName()}); //Audit Issue: User marked 'P-059' string as Not an Issue string with comment: ''.
    }
    ...
    private void readEncoding(boolean must)
        ...
        for (int i = 0; i < name.length(); i++) {
            ...
            if (i != 0
                ...
                continue;
            fatal("P-060", new Object[]{new Character(c)}); //Audit Issue: User marked 'P-060' string as Not an Issue string with comment: ''.
        }
        ...
        if (currentEncoding != null
            ...
            warning("P-061", new Object[]{name, currentEncoding}); //Audit Issue: User marked 'P-061' string as Not an Issue string with comment: ''.
    }
    ...
    private boolean maybeNotationDecl()
        ...
        String name = getMarkupDeclname("F-019", false); //Audit Issue: User marked 'F-019' string as Not an Issue string with comment: ''.
        ExternalEntity entity = new ExternalEntity(in);

        whitespace("F-011"); //Audit Issue: User marked 'F-011' string as Not an Issue string with comment: ''.
        if (peek("PUBLIC")) {
            whitespace("F-009"); //Audit Issue: User marked 'F-009' string as Not an Issue string with comment: ''.
            ...
            fatal("P-062"); //Audit Issue: User marked 'P-062' string as Not an Issue string with comment: ''.
        maybeWhitespace();
        nextChar('>', "F-032", name);
        if (start != in)
            error("V-013", null); //Audit Issue: User marked 'V-013' string as Not an Issue string with comment: ''.
        if (entity.systemId != null && entity.systemId.indexOf('#') != -1)
            error("P-056", new Object[]{entity.systemId}); //Audit Issue: User marked 'P-056' string as Not an Issue string with comment: ''.

        Object value = notations.get(name);
        if (value != null && value instanceof ExternalEntity)
            warning("P-063", new Object[]{name}); //Audit Issue: User marked 'P-063' string as Not an Issue string with comment: ''.
    ...
    private char getc() throws IOException, SAXException {
        ...
        while (in.isEOF()) {
            if (in.isInternal() || (doLexicalPE && !in.isDocument()))
                ...
                fatal("P-064", new Object[]{in.getName()}); //Audit Issue: User marked 'P-064' string as Not an Issue string with comment: ''.
            }
        }
        if ((c = in.getc()) == '%' && doLexicalPE) {
            ...
            // see note in maybePEReference re making this be nonfatal.
                fatal("V-022"); //Audit Issue: User marked 'V-022' string as Not an Issue string with comment: ''.
            else
                throw new InternalError();
            pushReader(" ".toCharArray(), null, false);
            return in.getc();
    ...
    private void nextChar(char c, String location, String near)
        ...
        while (in.isEOF() && !in.isDocument())
            in = in.pop();
        if (!in.peekc(c))
            fatal("P-008", new Object[] //Audit Issue: User marked 'P-008' string as Not an Issue string with comment: ''.
            {new Character(c),
    ...
    private boolean pushReader(ExternalEntity next)
        ...
        try {
            ...
            if (next.publicId != null)
                msg += " (public id:" + next.publicId + ")"; //Audit Issue: User marked ' (public id:' string as Not an Issue string with comment: ''.
