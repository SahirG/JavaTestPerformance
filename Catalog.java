...
public class Catalog {
  ...
  public static final int OVERRIDE = CatalogEntry.addEntryType("OVERRIDE", 1); //Audit Issue: User marked 'OVERRIDE' string as Not an Issue string with comment: ''.
  ...
  public String getDefaultOverride() {
    if (default_override) {
      return "yes"; //Audit Issue: User marked 'yes' string as Not an Issue string with comment: ''.
    } else {
      return "no";
    }
  }
  ...
  public synchronized void parseCatalog(String mimeType, InputStream is)
    ...
    default_override = catalogManager.getPreferPublic();
    catalogManager.debug.message(4, "Parse " + mimeType + " catalog on input stream"); //Audit Issue: User marked ' catalog on input stream' string as Not an Issue string with comment: ''. //Audit Issue: User marked ' catalog on input stream' string as Not an Issue string with comment: ''. //Audit Issue: User marked ' catalog on input stream' string as Not an Issue string with comment: ''. //Audit Issue: User marked ' catalog on input stream' string as Not an Issue string with comment: ''. //Audit Issue: User marked ' catalog on input stream' string as Not an Issue string with comment: ''.
  ...
  public void addEntry(CatalogEntry entry) {
    ...
    if (type == BASE) {
      ...
      catalogManager.debug.message(4, "OVERRIDE", entry.getEntryArg(0)); //Audit Issue: User marked 'OVERRIDE' string as Not an Issue string with comment: ''.
  ...
  public String resolveDoctype(String entityName,
    ...
    if (systemId != null && systemId.startsWith("urn:publicid:")) {
      ...
      if (publicId != null && !publicId.equals(systemId)) {
        catalogManager.debug.message(1, "urn:publicid: system identifier differs from public identifier; using public identifier"); //Audit Issue: User marked 'urn:publicid: system identifier differs from public identifier; using public identifier' string as Not an Issue string with comment: ''.
        systemId = null;
      } else {
        publicId = systemId;
        systemId = null;
    ...
    while (en.hasMoreElements()) {
      ...
      if (e.getEntryType() == OVERRIDE) {
        over = e.getEntryArg(0).equalsIgnoreCase("YES"); //Audit Issue: User marked 'YES' string as Not an Issue string with comment: ''.
        continue;
      }
  ...
  protected String makeAbsolute(String sysid) {
    ...
    try {
      ...
      catalogManager.debug.message(1, "Malformed URL on system identifier", sysid); //Audit Issue: User marked 'Malformed URL on system identifier' string as Not an Issue string with comment: ''.
    }
