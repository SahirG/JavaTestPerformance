               ...
               DTDHandler, DeclHandler, DOMSerializer, Serializer
{
    ...
    public boolean reset()
    {
        if ( _elementStateCount > 1 ) {
            String msg = DOMMessageFormatter.formatMessage(DOMMessageFormatter.SERIALIZER_DOMAIN,
                                                           "ResetInMiddle", null); //Audit Issue: User marked 'ResetInMiddle' string as Not an Issue string with comment: ''.
        ...
        throws SAXException
    {
        try {
        _printer.enterDTD();
        _printer.printText( "<!ENTITY " ); //Audit Issue: User marked '<!ENTITY ' string as Not an Issue string with comment: ''.
        _printer.printText( name );
        _printer.printText( " \"" );
        printEscaped( value );
        _printer.printText( "\">" );
        ...
        case Node.DOCUMENT_NODE : {
            ...
            if (docType != null) {
                ...
                // DOM Level 1 -- does implementation have methods?
                    ...
                    try {
                        java.lang.reflect.Method getPublicId = docTypeClass.getMethod("getPublicId", (Class[]) null); //Audit Issue: User marked 'getPublicId' string as Not an Issue string with comment: ''.
                    ...
                    try {
                        java.lang.reflect.Method getSystemId = docTypeClass.getMethod("getSystemId", (Class[]) null); //Audit Issue: User marked 'getSystemId' string as Not an Issue string with comment: ''.
                ...
                if (fDOMErrorHandler != null) {
                    ...
                    if ((features & DOMSerializerImpl.SPLITCDATA) == 0) {
                        ...
                        if ((features & DOMSerializerImpl.WELLFORMED) != 0) {
                            // issue fatal error
                            modifyDOMError(msg, DOMError.SEVERITY_FATAL_ERROR, "wf-invalid-character", fCurrentNode); //Audit Issue: User marked 'wf-invalid-character' string as Not an Issue string with comment: ''.
