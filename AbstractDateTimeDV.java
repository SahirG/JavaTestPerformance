...
public abstract class AbstractDateTimeDV extends TypeValidator {
    ...
    protected static final DatatypeFactory datatypeFactory = new DatatypeFactoryImpl();
        ...
        protected  void getTime (String buffer, int start, int end, DateTimeData data) throws RuntimeException{
                ...
                if (buffer.charAt(stop++)!=':') {
                        throw new RuntimeException("Error in parsing time zone" ); //Audit Issue: User marked 'Error in parsing time zone' string as Not an Issue string with comment: ''.
                }
                start = stop;
                stop = stop+2;
        ...
        protected int getYearMonth (String buffer, int start, int end, DateTimeData date) throws RuntimeException{
                ...
                if (length<4) {
                        ...
                        throw new RuntimeException("Leading zeros are required if the year value would otherwise have fewer than four digits; otherwise they are forbidden"); //Audit Issue: User marked 'Leading zeros are required if the year value would otherwise have fewer than four digits; otherwise they are forbidden' string as Not an Issue string with comment: ''.
                }
        ...
        protected void parseTimeZone (String buffer, int start, int end, DateTimeData date) throws RuntimeException{
                ...
                if ( start < end ) {
                        if (!isNextCharUTCSign(buffer, start, end)) {
                                throw new RuntimeException ("Error in month parsing"); //Audit Issue: User marked 'Error in month parsing' string as Not an Issue string with comment: ''.
                        }
                        else {
        ...
        protected void getTimeZone (String buffer, DateTimeData data, int sign, int end) throws RuntimeException{
            ...
            if(data.timezoneHr != 0 || data.timezoneMin != 0)
                ...
                if ( DEBUG ) {
                        System.out.println("time[hh]="+data.timezoneHr + " time[mm]=" +data.timezoneMin); //Audit Issue: User marked ' time[mm]=' string as Not an Issue string with comment: ''.
                }
        }
        ...
        protected void validateDateTime (DateTimeData data) {
                ...
                if (!Constants.SCHEMA_1_1_SUPPORT && data.year==0 ) {
                        throw new RuntimeException("The year \"0000\" is an illegal year value"); //Audit Issue: User marked 'The year \"0000\" is an illegal year value' string as Not an Issue string with comment: ''.

                }

                if ( data.month<1 || data.month>12 ) {
                        throw new RuntimeException("The month must have values 1 to 12"); //Audit Issue: User marked 'The month must have values 1 to 12' string as Not an Issue string with comment: ''.

                }

                //validate days
                if ( data.day>maxDayInMonthFor(data.year, data.month) || data.day<1 ) {
                        throw new RuntimeException("The day must have values 1 to 31"); //Audit Issue: User marked 'The day must have values 1 to 31' string as Not an Issue string with comment: ''.
                }
                ...
                if ( data.hour>23 || data.hour<0 ) {
                        if (data.hour == 24 && data.minute == 0 && data.second == 0) {
                                ...
                                throw new RuntimeException("Hour must have values 0-23, unless 24:00:00"); //Audit Issue: User marked 'Hour must have values 0-23, unless 24:00:00' string as Not an Issue string with comment: ''.
                        }
                }

                //validate
                if ( data.minute>59 || data.minute<0 ) {
                        throw new RuntimeException("Minute must have values 0-59"); //Audit Issue: User marked 'Minute must have values 0-59' string as Not an Issue string with comment: ''.
                }

                //validate
                if ( data.second>=60 || data.second<0 ) {
                        throw new RuntimeException("Second must have values 0-59"); //Audit Issue: User marked 'Second must have values 0-59' string as Not an Issue string with comment: ''.
    ...
    protected final boolean isNextCharUTCSign(String buffer, int start, int end) {
        ...
        protected void normalize(DateTimeData date) {
                ...
                if ( DEBUG ) {
                        System.out.println("==>date.minute"+date.minute); //Audit Issue: User marked '==>date.minute' string as Not an Issue string with comment: ''.
                        System.out.println("==>date.timezoneMin" +date.timezoneMin); //Audit Issue: User marked '==>date.timezoneMin' string as Not an Issue string with comment: ''.
                }
                ...
                if ( DEBUG ) {
                        System.out.println("==>carry: " + carry); //Audit Issue: User marked '==>carry: ' string as Not an Issue string with comment: ''.
                }
                //add hours
                ...
                if ( DEBUG ) {
                        System.out.println("==>date.hour"+date.hour); //Audit Issue: User marked '==>date.hour' string as Not an Issue string with comment: ''.
                        System.out.println("==>carry: " + carry); //Audit Issue: User marked '==>carry: ' string as Not an Issue string with comment: ''.
                }
    ...
    /**
        ...
        if (value == Integer.MIN_VALUE) {
            ...
            return;
                ...
                if (nch == 4) {
                        if (value < 10)
                                message.append("000"); //Audit Issue: User marked '000' string as Not an Issue string with comment: ''.
                        else if (value < 100)
                                message.append("00"); //Audit Issue: User marked '00' string as Not an Issue string with comment: ''.
                        else if (value < 1000)
                                message.append('0');
                        message.append(value);
                }
    ...
    private void append3(StringBuffer message, double value) {
        ...
        if (value < 1) {
            ...
            message.append("0."); //Audit Issue: User marked '0.' string as Not an Issue string with comment: ''.
            for (int i = 1; i < exp; ++i) {
                message.append('0');
            }
