        ...
        MessageHandler
{
    ...
    private void resumeSelect(Message header)
    {
        ...
        if (transportDebug()) {
            ...
            dprint(".resumeSelect: id/" //Audit Issue: User marked '.resumeSelect: id/' string as Not an Issue string with comment: ''.
                   + requestId
                   + " " + getConnection()
                   );
    ...
    {
        try {
            if (transportDebug()) dprint(".REQUEST 1.0->: " + header); //Audit Issue: User marked '.REQUEST 1.0->: ' string as Not an Issue string with comment: ''.
            try {
            ...
            if (transportDebug())
                dprint(".REQUEST 1.0: !!ERROR!!: " + header, t); //Audit Issue: User marked '.REQUEST 1.0: !!ERROR!!: ' string as Not an Issue string with comment: ''.
    ...
    public void handleInput(RequestMessage_1_2 header) throws IOException
    {
        try {
            try {
                ...
                if (transportDebug()) dprint(".REQUEST 1.2->: id/" //Audit Issue: User marked '.REQUEST 1.2->: id/' string as Not an Issue string with comment: ''.
                                             + header.getRequestId()
                                             + ": "
                                             + header);
    ...
    public void handleInput(ReplyMessage_1_0 header) throws IOException
    {
        try {
            try {
                if (transportDebug()) dprint(".REPLY 1.0->: " + header); //Audit Issue: User marked '.REPLY 1.0->: ' string as Not an Issue string with comment: ''.
            ...
            if (transportDebug())dprint(".REPLY 1.0: !!ERROR!!: " + header, t); //Audit Issue: User marked '.REPLY 1.0: !!ERROR!!: ' string as Not an Issue string with comment: ''.
            // Mask the exception from thread.;
        } finally {
            if (transportDebug()) dprint(".REPLY 1.0<-: " + header); //Audit Issue: User marked '.REPLY 1.0<-: ' string as Not an Issue string with comment: ''.
        }
    ...
    public void handleInput(ReplyMessage_1_2 header) throws IOException
    {
        try {
            try {
                ...
                if (transportDebug()) {
                    dprint(".REPLY 1.2->: id/" //Audit Issue: User marked '.REPLY 1.2->: id/' string as Not an Issue string with comment: ''.
                           + + header.getRequestId()
            ...
            if (transportDebug()) dprint(".REPLY 1.2: id/" //Audit Issue: User marked '.REPLY 1.2: id/' string as Not an Issue string with comment: ''.
                                         + header.getRequestId()
                                         + ": !!ERROR!!: "
                                         + header, t);
            // Mask the exception from thread.;
    ...
    public void handleInput(LocateRequestMessage_1_0 header) throws IOException
    {
        try {
            if (transportDebug())
                dprint(".LOCATE_REQUEST 1.0->: " + header); //Audit Issue: User marked '.LOCATE_REQUEST 1.0->: ' string as Not an Issue string with comment: ''.
            try {
                messageHeader = header;
                setInputObject();
            } finally {
            ...
            if (transportDebug())
                dprint(".LOCATE_REQUEST 1.0: !!ERROR!!: " + header, t); //Audit Issue: User marked '.LOCATE_REQUEST 1.0: !!ERROR!!: ' string as Not an Issue string with comment: ''.
            // Mask the exception from thread.;
        } finally {
            if (transportDebug())
                dprint(".LOCATE_REQUEST 1.0<-: " + header); //Audit Issue: User marked '.LOCATE_REQUEST 1.0<-: ' string as Not an Issue string with comment: ''.
    ...
    public void handleInput(LocateRequestMessage_1_2 header) throws IOException
    {
        try {
            try {
                ...
                if (transportDebug())
                    dprint(".LOCATE_REQUEST 1.2->: id/" //Audit Issue: User marked '.LOCATE_REQUEST 1.2->: id/' string as Not an Issue string with comment: ''.
                           + header.getRequestId()
                           + ": "
                           + header);
    ...
    public void handleInput(LocateReplyMessage_1_0 header) throws IOException
    {
        try {
            if (transportDebug())
                dprint(".LOCATE_REPLY 1.0->:" + header); //Audit Issue: User marked '.LOCATE_REPLY 1.0->:' string as Not an Issue string with comment: ''.
            try {
                messageHeader = header;
                setInputObject();
            ...
            if (transportDebug())
                dprint(".LOCATE_REPLY 1.0: !!ERROR!!: " + header, t); //Audit Issue: User marked '.LOCATE_REPLY 1.0: !!ERROR!!: ' string as Not an Issue string with comment: ''.
            // Mask the exception from thread.;
        } finally {
            if (transportDebug())
                dprint(".LOCATE_REPLY 1.0<-: " + header); //Audit Issue: User marked '.LOCATE_REPLY 1.0<-: ' string as Not an Issue string with comment: ''.
    ...
    public void handleInput(LocateReplyMessage_1_2 header) throws IOException
    {
        try {
            try {
                ...
                if (transportDebug()) dprint(".LOCATE_REPLY 1.2->: id/" //Audit Issue: User marked '.LOCATE_REPLY 1.2->: id/' string as Not an Issue string with comment: ''.
                                             + header.getRequestId()
                                             + ": "
                                             + header);
            ...
            if (transportDebug())
                dprint(".LOCATE_REPLY 1.2: id/" //Audit Issue: User marked '.LOCATE_REPLY 1.2: id/' string as Not an Issue string with comment: ''.
                       + header.getRequestId()
                       + ": !!ERROR!!: "
                       + header, t);
            // Mask the exception from thread.;
    ...
    public void handleInput(FragmentMessage_1_2 header) throws IOException
    {
        try {
            try {
                ...
                if (transportDebug()) {
                    dprint(".FRAGMENT 1.2->: id/" //Audit Issue: User marked '.FRAGMENT 1.2->: id/' string as Not an Issue string with comment: ''.
                           + header.getRequestId()
                           + ": more?: " + header.moreFragmentsToFollow()
                           + ": " + header);
                }
            ...
            if (transportDebug()) dprint(".FRAGMENT 1.2<-: id/" //Audit Issue: User marked '.FRAGMENT 1.2<-: id/' string as Not an Issue string with comment: ''.
                                         + header.getRequestId()
                                         + ": "
                                         + header);
        }
    ...
    public void handleInput(CancelRequestMessage header) throws IOException
    {
        try {
            try {
                ...
                if (transportDebug()) dprint(".CANCEL->: id/" //Audit Issue: User marked '.CANCEL->: id/' string as Not an Issue string with comment: ''.
                                             + header.getRequestId() + ": "
    ...
    protected void handleRequestRequest(CorbaMessageMediator messageMediator)
    {
        ...
        if (orb.subcontractDebugFlag) {
            ...
            dprint( ".handleRequest: " + opAndId(messageMediator)
                    + ": dispatching to scid: " + oktemp.getSubcontractId()); //Audit Issue: User marked ': dispatching to scid: ' string as Not an Issue string with comment: ''.
        }
