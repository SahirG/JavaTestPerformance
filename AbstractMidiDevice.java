...
abstract class AbstractMidiDevice implements MidiDevice, ReferenceCountingDevice {
    ...
    protected AbstractMidiDevice(MidiDevice.Info info) {
        ...
        if(Printer.trace) Printer.trace(">> AbstractMidiDevice CONSTRUCTOR"); //Audit Issue: User marked '>> AbstractMidiDevice CONSTRUCTOR' string as Not an Issue string with comment: ''.

        this.info = info;
        openRefCount = 0;

        if(Printer.trace) Printer.trace("<< AbstractMidiDevice CONSTRUCTOR completed"); //Audit Issue: User marked '<< AbstractMidiDevice CONSTRUCTOR completed' string as Not an Issue string with comment: ''.
    }
    ...
    public void open() throws MidiUnavailableException {
        if (Printer.trace) Printer.trace("> AbstractMidiDevice: open()"); //Audit Issue: User marked '> AbstractMidiDevice: open()' string as Not an Issue string with comment: ''.
        synchronized(this) {
            openRefCount = -1;
            doOpen();
        }
        if (Printer.trace) Printer.trace("< AbstractMidiDevice: open() completed"); //Audit Issue: User marked '< AbstractMidiDevice: open() completed' string as Not an Issue string with comment: ''.
    }
    ...
    private void openInternal(Object object) throws MidiUnavailableException {
        if (Printer.trace) Printer.trace("> AbstractMidiDevice: openInternal()"); //Audit Issue: User marked '> AbstractMidiDevice: openInternal()' string as Not an Issue string with comment: ''.
        synchronized(this) {
            if (openRefCount != -1) {
                openRefCount++;
        ...
        if (Printer.trace) Printer.trace("< AbstractMidiDevice: openInternal() completed"); //Audit Issue: User marked '< AbstractMidiDevice: openInternal() completed' string as Not an Issue string with comment: ''.
    }


    private void doOpen() throws MidiUnavailableException {
        if (Printer.trace) Printer.trace("> AbstractMidiDevice: doOpen()"); //Audit Issue: User marked '> AbstractMidiDevice: doOpen()' string as Not an Issue string with comment: ''.
        synchronized(this) {
            if (! isOpen()) {
                implOpen();
                open = true;
        ...
        if (Printer.trace) Printer.trace("< AbstractMidiDevice: doOpen() completed"); //Audit Issue: User marked '< AbstractMidiDevice: doOpen() completed' string as Not an Issue string with comment: ''.
    }


    public void close() {
        if (Printer.trace) Printer.trace("> AbstractMidiDevice: close()"); //Audit Issue: User marked '> AbstractMidiDevice: close()' string as Not an Issue string with comment: ''.
        synchronized (this) {
            doClose();
            openRefCount = 0;
        }
        if (Printer.trace) Printer.trace("< AbstractMidiDevice: close() completed"); //Audit Issue: User marked '< AbstractMidiDevice: close() completed' string as Not an Issue string with comment: ''.
    }
    ...
    public void closeInternal(Object object) {
        if (Printer.trace) Printer.trace("> AbstractMidiDevice: closeInternal()"); //Audit Issue: User marked '> AbstractMidiDevice: closeInternal()' string as Not an Issue string with comment: ''.
        synchronized(this) {
        ...
        if (Printer.trace) Printer.trace("< AbstractMidiDevice: closeInternal() completed"); //Audit Issue: User marked '< AbstractMidiDevice: closeInternal() completed' string as Not an Issue string with comment: ''.
    }


    public void doClose() {
        if (Printer.trace) Printer.trace("> AbstractMidiDevice: doClose()"); //Audit Issue: User marked '> AbstractMidiDevice: doClose()' string as Not an Issue string with comment: ''.
        synchronized(this) {
            if (isOpen()) {
                implClose();
                open = false;
        ...
        if (Printer.trace) Printer.trace("< AbstractMidiDevice: doClose() completed"); //Audit Issue: User marked '< AbstractMidiDevice: doClose() completed' string as Not an Issue string with comment: ''.
    }
    ...
    */
    protected Receiver createReceiver() throws MidiUnavailableException {
        throw new MidiUnavailableException("MIDI IN receiver not available"); //Audit Issue: User marked 'MIDI IN receiver not available' string as Not an Issue string with comment: ''.
    }
    ...
    */
    protected Transmitter createTransmitter() throws MidiUnavailableException {
        throw new MidiUnavailableException("MIDI OUT transmitter not available"); //Audit Issue: User marked 'MIDI OUT transmitter not available' string as Not an Issue string with comment: ''.
    }
                ...
                synchronized(transmitters) {
                    ...
                    if (optimizedReceiverCount == size) {
                        if (midiOutReceiver != null) {
                            if (TRACE_TRANSMITTER) Printer.println("Sending packed message to MidiOutReceiver"); //Audit Issue: User marked 'Sending packed message to MidiOutReceiver' string as Not an Issue string with comment: ''.
        ...
        */
        void sendMessage(MidiMessage message, long timeStamp) {
            ...
            synchronized(transmitters) {
                ...
                if (optimizedReceiverCount == size) {
                    if (midiOutReceiver != null) {
                        if (TRACE_TRANSMITTER) Printer.println("Sending MIDI message to MidiOutReceiver"); //Audit Issue: User marked 'Sending MIDI message to MidiOutReceiver' string as Not an Issue string with comment: ''.
                        midiOutReceiver.send(message, timeStamp);
                    }
                } else {
                    if (TRACE_TRANSMITTER) Printer.println("Sending MIDI message to "+size+" transmitter's receivers"); //Audit Issue: User marked 'Sending MIDI message to ' string as Not an Issue string with comment: ''.
                    for (int i = 0; i < size; i++) {
                        Receiver receiver = ((Transmitter)transmitters.get(i)).getReceiver();
                        if (receiver != null) {
