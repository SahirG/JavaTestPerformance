...
class PackageReader extends BandStructure {
    ...
    void read() throws IOException {
        ...
        try {
            ...
            if (verbose > 0)
                Utils.log.info("Stream offsets:"+ //Audit Issue: User marked 'Stream offsets:' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Stream offsets:' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Stream offsets:' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Stream offsets:' string as Not an Issue string with comment: ''. //Audit Issue: User marked 'Stream offsets:' string as Not an Issue string with comment: ''.
    ...
    void readConstantPool() throws IOException {
        ...
        for (int k = 0; k < ConstantPool.TAGS_IN_ORDER.length; k++) {
            ...
            if (optDumpBands) {
                try (PrintStream ps = new PrintStream(getDumpStream(index, ".idx"))) { //Audit Issue: User marked '.idx' string as Not an Issue string with comment: ''.
                 ...
                 : new PrintStream(getDumpStream(attr_definition_headers, ".def")))
        {
            for (int i = 0; i < numAttrDefs; i++) {
                ...
                if (!pvLayout.equals(def.layout())) {
                    throw new IOException("Bad attribute layout in version 150 archive: "+def.layout()); //Audit Issue: User marked 'Bad attribute layout in version 150 archive: ' string as Not an Issue string with comment: ''.
                }
            ...
            for (int i = 0; i < codeOps.length; i++) {
                ...
                default:
                    if (Instruction.isInvokeInitOp(bc)) {
                        ...
                        MemberEntry ref = pkg.cp.getOverloadingForIndex(CONSTANT_Methodref, classRef, "<init>", coding); //Audit Issue: User marked '<init>' string as Not an Issue string with comment: ''.
