...
final class CipherCore {
    ...
    private static int getNumOfUnit(String mode, int offset, int blockSize)
        ...
        if (mode.length() > offset) {
            ...
            if ((numInt % 8 != 0) || (result > blockSize)) {
                throw new NoSuchAlgorithmException
                    ("Invalid algorithm mode: " + mode); //Audit Issue: User marked 'Invalid algorithm mode: ' string as Not an Issue string with comment: ''.
            }
        }
        return result;
    }
        ...
        throws NoSuchPaddingException
    {
        ...
        if (paddingScheme.equalsIgnoreCase("NoPadding")) {
            padding = null;
        } else if (paddingScheme.equalsIgnoreCase("ISO10126Padding")) { //Audit Issue: User marked 'ISO10126Padding' string as Not an Issue string with comment: ''.
        ...
        if (iv != null) {
            ...
            try {
                ...
                throw new RuntimeException("Cannot find " + algName +
                    " AlgorithmParameters implementation in SunJCE provider"); //Audit Issue: User marked ' AlgorithmParameters implementation in SunJCE provider' string as Not an Issue string with comment: ''.
            } catch (NoSuchProviderException nspe) {
                // should never happen
                throw new RuntimeException("Cannot find SunJCE provider"); //Audit Issue: User marked 'Cannot find SunJCE provider' string as Not an Issue string with comment: ''.
            }
            try {
                params.init(ivSpec);
